...
const tabLabel = (intl, tab) => {
  let key = null;
  if (tab === GENERAL) {
    key = 'EditEquipmentListingWizard.tabLabelGeneral';
  } else if (tab === LOCATION) {
    key = 'EditEquipmentListingWizard.tabLabelLocation';
  } else if (tab === PRICING) {
    key = 'EditEquipmentListingWizard.tabLabelPricing';
  } else if (tab === AVAILABILITY) {
    key = 'EditEquipmentListingWizard.tabLabelAvailability';
  } else if (tab === PHOTOS) {
    key = 'EditEquipmentListingWizard.tabLabelPhotos';
  }
...
class EditEquipmentListingWizard extends Component {
  constructor(props) {
    super(props);
  ...
  handlePayoutSubmit(values) {
    this.props
      ...
      .then(response => {
        this.props.onManageDisableScrolling('EditEquipmentListingWizard.payoutModal', false);
      })
      .catch(() => {
        // do nothing
      });
...
EditEquipmentListingWizard.defaultProps = {
  className: null,
  currentUser: null,
  rootClassName: null,
  listing: null,
...
EditEquipmentListingWizard.propTypes = {
